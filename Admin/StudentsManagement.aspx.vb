...
Partial Class Admin_Students
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim lblActive As System.Web.UI.HtmlControls.HtmlGenericControl = (Master.FindControl("menuLIAdminStudents"))
        lblActive.Attributes("Class") = "treeview active"
    End Sub

    Protected Sub StudentsView_SelectedIndexChanged(sender As Object, e As EventArgs) Handles StudentsView.SelectedIndexChanged
        Try
            Dim db = New LinqDBClassesDataContext
            Dim qryStd = From m In db.Students
                         Select m
                         Where m.IDNo = StudentsView.SelectedValue.ToString

            For Each q In qryStd
                'MsgBox(q.IDNo)
                Dim paths As String() = New String() {"~/Students/Uploads/" + q.IDNo}
    ...
    Protected Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged
        DataSourceStudents.SelectCommand = "SELECT * FROM [Student] WHERE Memo_Fa Like '%" + txtSearch.Text + "%' OR Memo_En Like '%" + txtSearch.Text + "%' OR Name_Fa Like '%" + txtSearch.Text + "%' OR Family_Fa Like '%" + txtSearch.Text + "%' OR IDNo Like '%" + txtSearch.Text + "%' OR Evidence_Fa Like '%" + txtSearch.Text + "%'" 'IDCat=@IDCat  ORDER BY IDCat DESC"
    End Sub
    Protected Sub btnShowAll_Click(sender As Object, e As EventArgs) Handles btnShowAll.Click
        DataSourceStudents.SelectCommand = "SELECT * FROM [Student] ORDER BY IDS DESC" 'IDCat=@IDCat  ORDER BY IDCat DESC"
